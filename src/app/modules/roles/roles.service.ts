...
const updateRoleIntoDB = async (payload: TRole, email: string, id: string) => {
  ...
  const newPermissions: TPermission[] = defaultPermissions?.map(
    (permission) => {
      const payloadPermission = payload.permissions.find(
        (p) => p.feature === permission.feature
      );
      const existingPermission = thisRole.permissions.find(
        (p) => p.feature === permission.feature
      );

      if (
        payloadPermission &&
        Object.values(EAppFeatures).includes(payloadPermission.feature)
      ) {
        return {
        ...
        existingPermission &&
        Object.values(EAppFeatures).includes(existingPermission.feature)
      ) {
        return {
          feature: permission.feature,
          access: {
            ...
            create:
              existingPermission.access.create ?? permission.access.create,
            update:
              existingPermission.access.update ?? permission.access.update,
            delete:
              existingPermission.access.delete ?? permission.access.delete,
          },
        };
      } else {
        return {
